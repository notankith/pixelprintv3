...
export const CanvasEditor = forwardRef<CanvasEditorHandle, CanvasEditorProps>(function CanvasEditor({
  ...
  exportPDF: async (filename = 'design.pdf') => {
      ...
      try {
        ...
        // Pre-process images for better CORS handling
        console.log('Pre-processing images for CORS compatibility...');
        ...
        for (const obj of objects) {
          if ((obj as any).type === 'image') {
            ...
            if (imgObj._element) {
              // Try to set crossOrigin if not already set
              try {
                if (!imgObj._element.crossOrigin) {
                  imgObj._element.crossOrigin = 'anonymous';
                }
              } catch (e) {
                console.log('Could not set crossOrigin on image:', e);
              }
            }
          }
        }
        ...
        if (hasImages) {
          console.log('🖼️ Images detected, using enhanced export methods');
          // Give a moment for crossOrigin to take effect
        ...
        // Method 1: Try Fabric.js toDataURL first with image handling
        try {
          // Force render all objects including images
          fabricCanvas.renderAll();
  ...
  print: async () => {
      ...
      try {
        ...
        // Method 1: Try Fabric.js toDataURL (most reliable for text-only)
        try {
          console.log('Attempting Fabric toDataURL...');
          // Force render all objects including images
          fabricCanvas.renderAll();
          
          // First try with crossOrigin anonymous for images
          const objects = fabricCanvas.getObjects();
          for (const obj of objects) {
            if ((obj as any).type === 'image') {
              const imgObj = obj as any;
              if (imgObj._element && imgObj._element.crossOrigin !== 'anonymous') {
                imgObj._element.crossOrigin = 'anonymous';
              }
            }
          }
          ...
          dataUrl = fabricCanvas.toDataURL({ 
            format: 'png', 
            multiplier: 1, // Use 1:1 scale to match exactly what's visible
            left: 0,
            top: 0,
            width: fabricCanvas.getWidth(),
            height: fabricCanvas.getHeight()
          ...
          exportMethod = 'fabric-toDataURL';
          console.log('✅ Fabric toDataURL succeeded');
        } catch (fabricError) {
          console.log('❌ Fabric toDataURL failed:', fabricError);
          lastError = fabricError;
          
          // Method 2: Try html2canvas on canvas element with minimal options
          try {
            ...
            const html2canvasResult = await html2canvas(canvasElement, {
              allowTaint: true,
              useCORS: false,
              scale: 1, // Use 1:1 scale for exact match
              backgroundColor: '#ffffff',
              logging: false,
            ...
            try {
              ...
              try {
                ...
                console.log('✅ Canvas copy succeeded');
              } catch (copyError) {
                console.log('❌ Canvas copy failed, trying text-only render:', copyError);
                
                // Method 4: Manual text rendering with exact positioning and image handling
                console.log('Attempting manual rendering with images...');
                const objects = fabricCanvas.getObjects();
                ...
                for (const obj of objects) {
                  try {
                    if ((obj as any).type === 'image') {
                      const imgObj = obj as any;
                      console.log('Rendering image object:', imgObj);
                      ...
                      if (imgObj._element) {
                        ...
                        // Draw the image
                        const imgWidth = imgObj.width || imgObj._element.width || 100;
                        const imgHeight = imgObj.height || imgObj._element.height || 100;
                        ...
                        try {
                          ...
                          console.log('❌ Failed to render image, drawing placeholder:', imgError);
                ...
                dataUrl = newCanvas.toDataURL('image/png', 0.95);
                exportMethod = 'manual-render-with-images';
                console.log('✅ Manual rendering with images succeeded');
              }
            } catch (manualError) {
              ...
              console.log('Using emergency text-only fallback...');
              const pdf = new jsPDF({ 
        ...
        if (!dataUrl) {
          throw new Error(`All export methods failed. Last error: ${lastError?.message || 'Unknown'}`);
        }
