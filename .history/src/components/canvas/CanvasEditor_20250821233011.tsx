...
export const CanvasEditor = forwardRef<CanvasEditorHandle, CanvasEditorProps>(function CanvasEditor({
      ...
      switch (element.type) {
        ...
        case 'image':
          if (element.properties.url) {
            Image.fromURL(element.properties.url)
              .then(img => {
                img.set({
                  left: element.x,
                  top: element.y,
  ...
  if (!canvas) { toast.error('Canvas not ready'); return; }
      try {
        ...
        // embed PDF in iframe for printing
        w.document.write(`<!doctype html><html><head><title>Print</title></head><body style="margin:0"><iframe src="${url}" style="border:0;width:100%;height:100vh;" /></body></html>`);
        w.document.close();
        w.focus();
        w.onload = () => {
          try {
