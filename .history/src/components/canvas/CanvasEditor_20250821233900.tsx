...
export const CanvasEditor = forwardRef<CanvasEditorHandle, CanvasEditorProps>(function CanvasEditor({
  ...
  print: async () => {
      ...
      try {
        ...
        // Use the same robust export logic as exportPDF
        try {
          dataUrl = fabricCanvas.toDataURL({ 
            format: 'png', 
            multiplier: 2,
          ...
          exportMethod = 'fabric-toDataURL';
          console.log('Successfully exported for print using Fabric toDataURL');
        } catch (fabricError) {
          console.log('Fabric toDataURL failed for print:', fabricError);
          ...
          try {
            ...
            dataUrl = html2canvasResult.toDataURL('image/png', 0.9);
            exportMethod = 'html2canvas-container';
            console.log('Successfully exported for print using html2canvas on container');
          } catch (html2canvasError) {
            console.log('html2canvas on container failed for print:', html2canvasError);
            ...
            try {
              ...
              dataUrl = html2canvasResult.toDataURL('image/png', 0.9);
              exportMethod = 'html2canvas-direct';
              console.log('Successfully exported for print using html2canvas directly');
            } catch (directError) {
              console.log('html2canvas direct failed for print:', directError);
              throw new Error('Unable to export canvas for printing - all methods failed');
            }
          }
        }
        
        if (!dataUrl) {
          throw new Error('Failed to generate canvas image for printing');
        }
